        ...
        ConnectionMonitor
{
    ...
    /**
     * Zero argument constructor. The parser is in a safe but useless state
     * parsing an empty string. Set the lexer or connection using
     * {@link #setLexer} or {@link #setConnection}.
     * 
     * @see #setLexer(Lexer)
     ...
     * @param fb The object to use when information, warning and error messages
    ...
    /**
     * Constructor for custom HTTP access. This would be used to create a parser
     * for a URLConnection that needs a special setup or negotiation
     * conditioning beyond what is available from the
     * {@link #getConnectionManager ConnectionManager}.
     * 
     * @param connection A fully conditioned connection. The connect() method
     *            will be called so it need not be connected yet.
     * @param fb The object to use for message communication.
     * @throws ParserException If the creation of the underlying Lexer cannot be
     *             performed.
     */
     ...
     * @param resource Either a URL, a filename or a string of HTML. The string
     *            is considered HTML if the first non-whitespace character is a
     *            &lt;. The use of a url or file is autodetected by first
     *            attempting to open the resource as a URL, if that fails it is
     *            assumed to be a file name. A standard HTTP GET is performed to
     ...
     * @throws ParserException If the resourceLocn argument does not resolve to
     *             a valid page or file.
    ...
    /**
     * Construct a parser using the provided lexer. A feedback object printing
     * to {@link #STDOUT System.out} is used. This would be used to create a
     * parser for special cases where the normal creation of a lexer on a
     * URLConnection needs to be customized.
     * 
     * @param lexer The lexer to draw characters from.
     */
    ...
    /**
     * Construct a parser using the provided URLConnection. This would be used
     * to create a parser for a URLConnection that needs a special setup or
     * negotiation conditioning beyond what is available from the
     * {@link #getConnectionManager ConnectionManager}. A feedback object
     * printing to {@link #STDOUT System.out} is used.
     * 
     * @see #Parser(URLConnection,ParserFeedback)
     * @param connection A fully conditioned connection. The connect() method
     *            will be called so it need not be connected yet.
    ...
    /**
     * Set the connection for this parser. This method creates a new
     * <code>Lexer</code> reading from the connection.
     * 
     * @param connection A fully conditioned connection. The connect() method
     *            will be called so it need not be connected yet.
     * @exception ParserException if the character set specified in the HTTP
     *                header is not supported, or an i/o exception occurs
     *                creating the lexer.
     * @see #setLexer
     * @see #getConnection
     * @exception IllegalArgumentException if <code>connection</code> is
     *                <code>null</code>.
     * @exception ParserException if a problem occurs in connecting.
     */
     ...
     * 
     * @return The current url. This is the URL for the current page. A string
     *         passed into the constructor or set via setURL may be altered, for
     *         example, a file name may be modified to be a URL.
     * @see Page#getUrl
     * @see #setURL
     */
     ...
     *             would have been seen had the new encoding been in force.
    ...
    /**
     * Get the encoding for the page this parser is reading from. This item is
     * set from the HTTP header but may be overridden by meta tags in the head,
     * so this may change after the head has been parsed.
     * 
    ...
    /**
     * Reset the parser to start from the beginning again. This assumes support
     * for a reset from the underlying {@link org.htmlparser.lexer.Source}
     * object.
     * <p>
     ...
     * @param filter The filter to apply to the parsed nodes, or
     *            <code>null</code> to retrieve all the top level nodes.
     * @throws ParserException If a parsing error occurs.
     */
    ...
    /**
     * Apply the given visitor to the current page. The visitor is passed to the
     * <code>accept()</code> method of each node in the page in a depth first
     * traversal. The visitor <code>beginParsing()</code> method is called prior
     * to processing the page and <code>finishedParsing()</code> is called after
     * the processing.
     * 
     * @param visitor The visitor to visit all nodes with.
     * @throws ParserException If a parse error occurs while traversing the page
     *             with the visitor.
     */
     ...
     * @throws ParserException If a error occurs in setting up the underlying
     *             Lexer.
     * @exception IllegalArgumentException if <code>inputHTML</code> is
     *                <code>null</code>.
     */
    ...
    /**
     * Called just prior to calling connect. Part of the ConnectionMonitor
     * interface, this implementation just sends the request header to the
     * feedback object if any.
     * 
    ...
    /**
     * Called just after calling connect. Part of the ConnectionMonitor
     * interface, this implementation just sends the response header to the
     * feedback object if any.
     * 
