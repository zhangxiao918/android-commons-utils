...
package com.bluestome.android.databases.dao.impl;
...
public class BaseImpl implements IBaseDAO {
    ...
    protected int getMaxId(final String tableName) {
        return getMaxId(tableName, 0);
    }

    /**
     * 查询某表中的最大id号
     */
    protected int getMaxId(final String tableName, final int step) {
        int maxId = 0;
        // List<String> list = query("SELECT max(" + ContactsColumns._ID +
        // ") FROM " + tableName,
        // new String[0]);
        ...
        // }
        // }
        // return step > 0 ? maxId + step : maxId;
        return maxId;
    }
     ...
     * @return
     */
    protected <T> ArrayList<T> queryWithSort(Class<T> beanClass, String sql, String[] selectionArgs) {
        Cursor cursor = null;
     ...
     * @return
     */
    protected ArrayList<String> query(String sql,
            String[] selectionArgs) {
        Cursor cursor = null;
     ...
     * @return
     */
    protected Map<String, String> queryMap(String sql,
            String[] selectionArgs) {
        Cursor cursor = null;
     ...
     * @throws Exception
     */
    protected <T> Map<String, T> query(Class<T> beanClass, String keyColumn, String sql,
            String[] selectionArgs) {
        Cursor cursor = null;
     ...
     * @throws Exception
     */
    private <T> Map<String, T> cursorToMap(Cursor cursor, Class<T> beanClass, String keyColumn) {
        ...
        while (cursor.moveToNext()) {
            try {
                instence = cursorToObject(cursor, beanClass, methods, columnNames);
                Method method = instence.getClass().getMethod("get" + keyColumn);
    ...
    private <T> T cursorToObject(Cursor cursor, Class<T> beanClass, Method[] methods,
            String[] columnNames) {
        T instence;
        try {
    ...
    protected int update(String table, ContentValues values, String whereClause, String[] whereArgs) {
        int rows = getSQLiteDatabase().update(table, values, whereClause, whereArgs);
        return rows;
    }
