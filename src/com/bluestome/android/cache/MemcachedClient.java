...
/**
 * This is a Memcached client for the Java platform available from <a
 * href="http:/www.danga.com/memcached/">http://www.danga.com/memcached/</a>. <br/>
...
public class MemcachedClient {
    ...
    /**
     * Initializes client object to defaults. This enables compression and sets
     * compression threshhold to 15 KB.
     */
    private void init() {
     ...
     * 
     * @param primitiveAsString if true, then store all primitives as their
     *            string value.
     */
     ...
     * requirements. If enabled, data will be stored in compressed form if it is<br/>
     ...
     * decompressed.
     * 
     * @param compressEnable <CODE>true</CODE> to enable compression,
     *            <CODE>false</CODE> to disable compression
     */
    ...
    /**
     * Sets the required length for data to be considered for compression. If
     * the length of the data to be stored is not equal or larger than this
     * value, it will not be compressed. This defaults to 15 KB.
     * 
     ...
     * optional hashcode. The item is immediately made non retrievable.<br/>
    ...
    /**
     * Stores data to cache. If data does not already exist for this key on the
     * server, or if the key is being<br/>
     ...
     */
    private boolean set(String cmdname, String key, Object value, Date expiry, Integer hashCode,
            boolean asString) {
        ...
        if (sock == null) {
            if (errorHandler != null)
                errorHandler.handleErrorOnSet(this,
                        new IOException("no socket to server available"), key);
            return false;
        }
        ...
        if (NativeHandler.isHandled(value)) {
            ...
            try {
                if (log.isInfoEnabled())
                    log.info("++++ serializing for key: " + key + " for class: "
                            + value.getClass().getName());
        ...
        try {
            ...
            if (STORED.equals(line)) {
                ...
                log.error("++++ error storing data in cache for key: " + key + " -- length: "
                        + val.length);
                log.error("++++ server response: " + line);
            }
        } catch (IOException e) {
    ...
    /**
     * Increments/decrements the value at the specified key by inc. Note that
     * the server uses a 32-bit unsigned integer, and checks for<br/>
    ...
    /**
     * Retrieve a key from the server, using a specific hash. If the data was
     * compressed or serialized when compressed, it will automatically<br/>
    ...
    public Object get(String key, Integer hashCode, boolean asString) {
        ...
        if (sock == null) {
            if (errorHandler != null)
                errorHandler.handleErrorOnGet(this,
                        new IOException("no socket to server available"), key);
            return null;
        }
    ...
    /**
     * Retrieve multiple objects from the memcache. This is recommended over
     * repeated calls to {@link #get(String) get()}, since it<br/>
    ...
    /**
     * Retrieve multiple keys from the memcache. This is recommended over
     * repeated calls to {@link #get(String) get()}, since it<br/>
     * is more efficient.<br/>
     * 
    ...
    /**
     * This method loads the data from cache into a Map. Pass a SockIO object
     * which is ready to receive data and a HashMap<br/>
     * to store the results.
     * 
     * @param sock socket waiting to pass back data
     * @param hm hashmap to store data into
     * @param asString if true, and if we are using NativehHandler, return
     *            string val
     * @throws IOException if io exception happens while reading from socket
     */
    private void loadMulti(LineInputStream input, Map<String, Object> hm, boolean asString)
            throws IOException {
    ...
    /**
     * Invalidates the entire cache. Will return true only if succeeds in
    ...
    /**
     * Invalidates the entire cache. Will return true only if succeeds in
     * clearing all servers. If pass in null, then will try to flush all
     * servers.
     * 
    ...
    /**
     * Retrieves stats for all servers. Returns a map keyed on the servername.
     * The value is another map which contains stats with stat name as key and
     * value as value.
     * 
     * @return Stats map
     */
    ...
    /**
     * Retrieves stats for passed in servers (or all servers). Returns a map
     * keyed on the servername. The value is another map which contains stats
     * with stat name as key and value as value.
     * 
     * @param servers string array of servers to retrieve stats from, or all if
     *            this is null
     * @return Stats map
     */
    ...
    /**
     * Retrieves stats items for all servers. Returns a map keyed on the
     * servername. The value is another map which contains item stats with
     * itemname:number:field as key and value as value.
     * 
     * @return Stats map
     */
    ...
    /**
     * Retrieves stats for passed in servers (or all servers). Returns a map
     * keyed on the servername. The value is another map which contains item
     * stats with itemname:number:field as key and value as value.
     * 
     * @param servers string array of servers to retrieve stats from, or all if
     *            this is null
     * @return Stats map
    ...
    /**
     * Retrieves stats items for all servers. Returns a map keyed on the
     * servername. The value is another map which contains slabs stats with
     * slabnumber:field as key and value as value.
     * 
     * @return Stats map
     */
    ...
    /**
     * Retrieves stats for passed in servers (or all servers). Returns a map
     * keyed on the servername. The value is another map which contains slabs
     * stats with slabnumber:field as key and value as value.
     * 
     * @param servers string array of servers to retrieve stats from, or all if
     *            this is null
     * @return Stats map
    ...
    /**
     * Retrieves items cachedump for all servers. Returns a map keyed on the
     * servername. The value is another map which contains cachedump stats with
     * the cachekey as key and byte size and unix timestamp as value.
     * 
    ...
    /**
     * Retrieves stats for passed in servers (or all servers). Returns a map
     * keyed on the servername. The value is another map which contains
     * cachedump stats with the cachekey as key and byte size and unix timestamp
     * as value.
     * 
     * @param servers string array of servers to retrieve stats from, or all if
     *            this is null
    ...
    protected final class NIOLoader {
        ...
        private final class Connection {
            ...
            public String toString() {
                return "Connection to " + sock.getHost() + " with " + incoming.size()
                        + " bufs; done is " + isDone;
            }
        }
        ...
        public void doMulti(boolean asString, Map<String, StringBuilder> sockKeys, String[] keys,
            ...
            try {
                ...
                for (Iterator<String> i = sockKeys.keySet().iterator(); i.hasNext();) {
                    ...
                    if (sock == null) {
                        if (errorHandler != null)
                            errorHandler.handleErrorOnGet(this.mc, new IOException(
                                    "no socket to server available"), keys);
                        return;
                    }
                ...
                if (log.isDebugEnabled())
                    log.debug("Disconnecting; numConns=" + numConns + "  timeRemaining="
                            + timeRemaining);
