 ...
 * memory savings: Currently the Memcached driver for Java supports the
 * setSerialize() option. This can increase performance in some situations but
 * has a few issues: Code that performs class casting will throw
 * ClassCastExceptions when setSerialize is enabled. For example: mc.set( "foo",
 * new Integer( 1 ) ); Integer output = (Integer)mc.get("foo"); Will work just
 * file when setSerialize is true but when its false will just throw a
 * ClassCastException. Also internally it doesn't support Boolean and since
 * toString is called wastes a lot of memory and causes additional performance
 * issue. For example an Integer can take anywhere from 1 byte to 10 bytes. Due
 * to the way the memcached slab allocator works it seems like a LOT of wasted
 * memory to store primitive types as serialized objects (from a performance and
 * memory perspective). In our applications we have millions of small objects
 * and wasted memory would become a big problem. For example a Serialized
 * Boolean takes 47 bytes which means it will fit into the 64byte LRU. Using 1
 * byte means it will fit into the 8 byte LRU thus saving 8x the memory. This
 * also saves the CPU performance since we don't have to serialize bytes back
 * and forth and we can compute the byte[] value directly. One problem would be
 * when the user calls get() because doing so would require the app to know the
 * type of the object stored as a bytearray inside memcached (since the user
 * will probably cast). If we assume the basic types are interned we could use
 * the first byte as the type with the remaining bytes as the value. Then on
 * get() we could read the first byte to determine the type and then construct
 * the correct object for it. This would prevent the ClassCastException I talked
 * about above. We could remove the setSerialize() option and just assume that
 * standard VM types are always internd in this manner. mc.set( "foo", new
 * Boolean.TRUE ); Boolean b = (Boolean)mc.get( "foo" ); And the type casts
 * would work because internally we would create a new Boolean to return back to
 * the client. This would reduce memory footprint and allow for a virtual
 * implementation of the Externalizable interface which is much faster than
 * Serialzation. Currently the memory improvements would be: java.lang.Boolean -
 * 8x performance improvement (now just two bytes) java.lang.Integer - 16x
 * performance improvement (now just 5 bytes) Most of the other primitive types
 * would benefit from this optimization. java.lang.Character being another
 * obvious example. I know it seems like I'm being really picky here but for our
 * application I'd save 1G of memory right off the bat. We'd go down from 1.152G
 * of memory used down to 144M of memory used which is much better IMO.
...
public class NativeHandler {
    ...
    public static boolean isHandled(Object value) {
        ...
        return (value instanceof Byte ||
                ...
                value instanceof StringBuilder || value instanceof byte[])
                ? true
                : false;
    }
