 ...
 * connections<br/>
 * to memcached servers. The pool must be initialized prior to use. This should
 * typically be early on<br/>
...
public class SockIOPool {
    ...
    public static final int NEW_COMPAT_HASH = 2; // new CRC32 based
                                                 ...
                                                 // algorithm (works with other
                                                 // clients)
    public static final int CONSISTENT_HASH = 3; // MD5 Based -- Stops thrashing
                                                 // when a server added or
                                                 // removed
    ...
    private int maxCreate = 1; // this will be initialized by pool when the pool
                               // is initialized
    ...
    private boolean failback = true; // only used if failover is also set ...
                                     // controls putting a dead server back into
                                     // rotation
    private boolean nagle = false; // enable/disable Nagle's algorithm
    private int hashingAlg = NATIVE_HASH; // default to using the native hash as
                                          // it is the fastest
    ...
    /**
     * Sets the list of weights to apply to the server list. This is an int
     * array with each element corresponding to an element<br/>
    ...
    /**
     * Sets the failback flag for the pool. If this is true and we have marked a
     * host as dead, will try to bring it back. If it is false, we will never
    ...
    /**
     * Sets the aliveCheck flag for the pool. When true, this will attempt to
     * talk to the server on every connection checkout to make sure the
     * connection is still valid. This adds extra network chatter and thus is
     * defaulted off. May be useful if you want to ensure you do not have any
     * problems talking to the server on a dead connection.
     * 
    ...
    /**
     * Sets the hashing algorithm we will use. The types are as follows.
    ...
    /**
     * Internal private hashing method. This is the original hashing algorithm
     * from other clients. Found to be slow and have poor distribution.
     * 
    ...
    /**
     * Internal private hashing method. This is the new hashing algorithm from
     * other clients. Found to be fast and have very good distribution. UPDATE:
     * This is dog slow under java
     * 
     * @param key
     * @return
     */
    ...
    /**
     * Internal private hashing method. MD5 based hash algorithm for use in the
    ...
    public void initialize() {
        ...
        synchronized (this) {
            ...
            maxCreate = (poolMultiplier > minConn) ? minConn : minConn / poolMultiplier; // only
                                                                                         // create
                                                                                         // up
                                                                                         // to
                                                                                         // maxCreate
    ...
    private void populateBuckets() {
        ...
        for (int i = 0; i < servers.length; i++) {
            ...
            if (log.isDebugEnabled())
                log.debug("+++ creating initial connections (" + initConn + ") for host: "
                        + servers[i]);

            for (int j = 0; j < initConn; j++) {
                SockIO socket = createSocket(servers[i]);
                if (socket == null) {
                    log.error("++++ failed to create connection to: " + servers[i] + " -- only "
                            + j + " created.");
                    break;
                }
                ...
                if (log.isDebugEnabled())
                    log.debug("++++ created and added socket: " + socket.toString() + " for host "
                            + servers[i]);
            }
        }
    }
    ...
    /**
     * Creates a new SockIO obj for the given server. If server fails to
     * connect, then return null and do not try<br/>
    ...
    protected SockIO createSocket(String host) {
        ...
        try {
            if (socket == null) {
                ...
                long expire = (hostDeadDur.containsKey(host)) ? (((Long) hostDeadDur.get(host))
                        .longValue() * 2) : 1000;
     ...
     * hashcode. Trys to get SockIO from pool. Fails over to additional pools in
    ...
    /**
     * Returns a SockIO object from the pool for the passed in host. Meant to be
     * called from a more intelligent method<br/>
     ...
     * THREADSAFE, SO BE CAREFUL WHEN USING! Internal utility method.
     * 
     ...
     */
    protected void removeSocketFromPool(Map<String, Map<SockIO, Long>> pool, String host,
            SockIO socket) {
        if (pool.containsKey(host)) {
            Map<SockIO, Long> sockets = pool.get(host);
            if (sockets != null)
     ...
     * IS NOT THREADSAFE, SO BE CAREFUL WHEN USING! Internal utility method.
     * 
    ...
    /**
     * Checks a SockIO object in with the pool. This will remove SocketIO from
     * busy pool, and optionally<br/>
     * add to avail pool.
     * 
    ...
    private void checkIn(SockIO socket, boolean addToAvail) {
        ...
        String host = socket.getHost();
        if (log.isDebugEnabled())
            log.debug("++++ calling check-in on socket: " + socket.toString() + " for host: "
                    + host);
    ...
    /**
     * Returns a socket to the avail pool. This is called from SockIO.close().
        ...
        for (Iterator<String> i = pool.keySet().iterator(); i.hasNext();) {
            ...
            for (Iterator<SockIO> j = sockets.keySet().iterator(); j.hasNext();) {
                ...
                try {
                    ...
                    log.error("++++ failed to trueClose socket: " + socket.toString()
                            + " for host: " + host);
                }
    ...
    /**
     * Shuts down the pool. Cleanly closes all sockets.<br/>
    ...
    /**
     * Starts the maintenance thread. This thread will manage the size of the
     * active pool<br/>
    ...
    /**
     * Runs self maintenance on all internal pools. This is typically called by
     * the maintenance thread to manage pool size.
     */
    ...
    /**
     * MemCached client for Java, utility class for Socket IO. This class is a
     * wrapper around a Socket and its streams.
     * 
     * @author greg whalin <greg@meetup.com>
    ...
    public static class SockIO implements LineInputStream {
         ...
         */
        public SockIO(SockIOPool pool, String host, int port, int timeout, int connectTimeout,
        ...
        public void trueClose(boolean addToDeadPool) throws IOException {
            ...
            if (in != null) {
                try {
                    ...
                    log.error("++++ error closing input stream for socket: " + toString()
                            + " for host: " + getHost());
                    log.error(ioe.getMessage(), ioe);
                    errMsg.append("++++ error closing input stream for socket: " + toString()
                            + " for host: " + getHost() + "\n");
            ...
            if (out != null) {
                try {
                    ...
                    log.error("++++ error closing output stream for socket: " + toString()
                            + " for host: " + getHost());
                    log.error(ioe.getMessage(), ioe);
                    errMsg.append("++++ error closing output stream for socket: " + toString()
                            + " for host: " + getHost() + "\n");
                    errMsg.append(ioe.getMessage());
                    err = true;
                }
            ...
            if (sock != null) {
                try {
                    ...
                    log.error("++++ error closing socket: " + toString() + " for host: "
                            + getHost());
                    log.error(ioe.getMessage(), ioe);
                    errMsg.append("++++ error closing socket: " + toString() + " for host: "
                            + getHost() + "\n");
                    errMsg.append(ioe.getMessage());
                    err = true;
                }
